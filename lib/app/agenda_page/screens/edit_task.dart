...
class _EditTaskPageState extends State<EditTaskPage> {
  ...
  void getNotes() async {
    ...
    if (allNotes[widget.listID].startTime.length < 6) {
      _startTimeHour.text = _startTimeShow.substring(0, 2);
      _startTimeMin.text = _startTimeShow.substring(3, 5);
    }

    _endTimeShow = allNotes[widget.listID].endTime;
    if (allNotes[widget.listID].endTime.length < 6) {
      print(allNotes[widget.listID].endTime.length);
      _endTimeHour.text = _endTimeShow.substring(0, 2);
      _endTimeMin.text = _endTimeShow.substring(3, 5);
    }
  }
  ...
  void updateTask() {
    if (widget.taskID != null) {
      _updateNote(Notes.withID(widget.taskID, _controllerTitle.text,
          _controllerDesc.text, dateToShow, _startTimeShow, _endTimeShow));
    } else {}
  }
}
