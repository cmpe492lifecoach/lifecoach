...
class Repository implements AuthBase {
  ...
  Future<User> createWithEmailAndPassword(String email, String password) async {
    if (appMode == AppMode.DEBUG) {
      return await _fakeAuthenticationService.createWithEmailAndPassword(
          email, password);
    } else {
      User _user = await _firebaseAuthService.createWithEmailAndPassword(
          email, password);
  ...
  Future<bool> updateHeight(String userID, String newHeight) async{
    if(appMode == AppMode.DEBUG){
      ...
      return await _fireStoreDBService.updateHeight(userID, newHeight);
    }
